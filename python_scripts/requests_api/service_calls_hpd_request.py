...
def make_request(conn, write_to_csv=False):
    ...
    source = "HPD"
    dob_url = 'https://data.cityofnewyork.us/resource/fhrw-4uyv.json?agency=HPD&$where=created_date between "' + \
        api_helpers.get_start_date(table, source) + '" and "' + api_helpers.get_today(table, source) + '"&'
