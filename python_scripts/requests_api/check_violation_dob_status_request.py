...
def update_row(c, api_row):
  ...
  resolution_violation = violations_seeds.resulted_in_violation(resolution_description)
  resolution_no_action = violations_seeds.took_no_action(resolution_description)
  unable_to_investigate = violations_seeds.unable_to_investigate_call(resolution_description)
  ...
  days_to_close = int(violations_seeds.calculate_days_to_close(date, closed_date)) if closed_date else None
  if status.lower() == "open":
    open_over_month = violations_seeds.is_open_over_month(status, date)
  else:
    open_over_month = violations_seeds.was_open_over_month(closed_date, date)
...
def api_request(table, source):
  url = 'https://data.cityofnewyork.us/resource/fhrw-4uyv.json?$where=(agency=\'DOB\' OR agency=\'HPD\') AND closed_date > "'+ api_helpers.get_previous_days(table, source, 30) + '"&'
