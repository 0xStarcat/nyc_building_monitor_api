...
def make_request(conn, write_to_csv=False):
  ...
  source = "DOB"
  dob_url = 'https://data.cityofnewyork.us/resource/fhrw-4uyv.json?agency=DOB&$where=created_date between "'+ api_helpers.get_next_day_to_request(conn, table, source) + '" and "' + api_helpers.get_today(table, source) + '"&'
