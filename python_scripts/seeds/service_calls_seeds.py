...
def calculate_days_to_close(date_open, date_closed):
    date_difference = datetime.datetime.strptime(date_closed, "%Y%m%d") - \
        datetime.datetime.strptime(date_open, "%Y%m%d")
...
def seed(c, service_calls_json, write_to_csv=False):
    ...
    for index, call in enumerate(service_calls_json):
        ...
        closed_date = str(datetime.datetime.strptime(call["closed_date"][:10], "%Y-%m-%d").strftime(
            "%Y%m%d")) if "status" in call and "closed_date" in call and call["status"].lower() != "open" and call["status"].lower() != "pending" else None
