...
def seed_virtual_table(c):
    c.execute('DROP TABLE IF EXISTS {tn}'.format(tn=virtual_table))
    c.execute(
        'CREATE VIRTUAL TABLE IF NOT EXISTS {tn} USING fts5(id, house_number, address, borough_name)'.format(tn=virtual_table))
...
def seed(c, building_json):
    ...
    for index, building in enumerate(building_json["features"]):
        ...
        try:
            representative_point = json.dumps(
                context.boundary_helpers.get_representative_point_geojson(building["geometry"]))
        except:
            print("  X No Geo")
            continue
        ...
        if foreign_keys == None:
            if "BBL" in building["properties"] and "CT2010" in building["properties"]:
                unassigned_buildings.append([str(building["properties"]["Address"]),
                                             int(building["properties"]["BBL"]), building["properties"]["CT2010"]])
            print("  * no CT", str(index) + "/" +
                  str(len(building_json["features"])), building["properties"]["Address"],)
            continue
    ...
    log_file = config.UNASSIGNED_BUILDINGS_FILE
    try:
