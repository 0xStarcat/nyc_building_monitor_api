...
# def update_entry(c, date):
  # unique index needed!!
def new_entry(c, entry_data):
  parsed_date = datetime.datetime.strptime(entry_data["date"], '%Y%m%d%H%M%S')
  ...
  c.execute('INSERT INTO {tn} ({col}) VALUES(?, ?, ?, ?, ?)'.format(tn=table, col=col1), str(parsed_date ,new_violations, new_service_calls, resolved_violations, resolved_service_calls))
