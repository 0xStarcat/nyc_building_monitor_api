...
def insertion_of_record_dob(c):
    ...
    try:
        ...
        entry = c.fetchone()
        assert entry[
            0] == 'The Department of Buildings investigated this complaint and closed it. If the problem still exists, please call 311 and file a new complaint. If you are outside of New York City, please call (212) NEW-YORK (212-639-9675).'
        ...
        c.execute('SELECT is_duplicate FROM {tn}'.format(tn=test_context.context.service_calls_seeds.table))
        entry = c.fetchone()
        assert entry[0] == 0
...
def test_call_is_duplicate():
    description = 'The Department of Buildings determined that the conditions described in this complaint were addressed under another service request number. Click on "Learn More" in the "Did You Know" section below for more information.'
    assert test_context.context.service_calls_seeds.call_is_duplicate(description) == True

    description = 'We have never seen a call like this in our lives'
    assert test_context.context.service_calls_seeds.call_is_duplicate(description) == False
